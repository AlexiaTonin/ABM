...
const verMasShow = (id) => {
	...
	const body = `
	<label class="fs-4"><b>Nombre:</b> ${usuarioAMostrar.nombre}</label>
	<br>
	<label class="fs-4"><b>Apellido:</b> ${usuarioAMostrar.apellido}</label>
	<br>
	<label class="fs-4"><b>DNI:</b> ${usuarioAMostrar.dni}</label>
	<br>
	<label class="fs-4"><b>Email:</b> ${usuarioAMostrar.email}</label>
	`;
	...
	if (usuariosFiltrados) {
		...
		for (const user of usuariosFiltrados) {
			...
			<tr>
				...
				<td>${user.dni}</td>
				<td>
					<button class="btn btn-outline-success" onclick="verMasShow('${user.id}')">Ver más</button>
				</td>
				<td>
					<button class="btn btn-outline-danger" onclick="borrar('${user.id}')">Borrar</button>
				</td>
				<td>
					<button class="btn btn-outline-warning" onclick="editar('${user.id}')">Editar</button>
				</td>
			</tr>
			`;
			trs.push(tr);
		...
		for (const user of usuarios) {
			...
			<tr>
				...
				<td>${user.dni}</td>
				<td>
					<button class='btn modalButton' onclick="verMasShow('${user.id}')">Ver más</button>
				</td>
				<td>
				<button class='btn btn-outline-warning' onclick="editar('${user.id}')">Editar</button>
				</td>
